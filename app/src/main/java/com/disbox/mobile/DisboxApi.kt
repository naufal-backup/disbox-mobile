...
class DisboxApi(private val context: Context, var webhookUrl: String) {
    ...
    suspend fun downloadFilePartial(
        ...
        out.use { stream ->
            for (i in startIndex until actualEndIndex) {
                ...
                var retries = 0
                while (chunkData == null && retries < 3) {
                    try {
                        chunkData = downloadSingleChunk(messageIds[i])
                    } catch (e: Exception) {
                        retries++
                        if (retries >= 3) throw e
                        kotlinx.coroutines.delay(1500L * retries)
                    }
                }
                if (chunkData != null) {
    ...
    suspend fun downloadSingleChunk(msgId: String): ByteArray = withContext(Dispatchers.IO) {
        ...
        while (retries < 3) {
            ...
            if (msgRes.code == 429) {
                val rBody = msgRes.body?.string()
                val map: Map<String, Any>? = try {
                    rBody?.let { gson.fromJson(it, object : TypeToken<Map<String, Any>>() {}.type) }
            ...
            if (!msgRes.isSuccessful) {
                msgRes.close()
                retries++
                if (retries >= 3) throw Exception("Failed to fetch msg $msgId after 3 retries")
            ...
            if (!chunkRes.isSuccessful) {
                chunkRes.close()
                retries++
                if (retries >= 3) throw Exception("Failed to download chunk after 3 retries")
                kotlinx.coroutines.delay(1000L * retries)
                continue
            }
            ...
            return@withContext chunkData
        }
        throw Exception("Max retries exceeded for $msgId")
    }
}
